// $(document).ready(function(){
    ...
    $('button#create').on('click', function(){
        alert('Hi')
        $('div#tree').attr("hidden", true)
        $('form#create').attr("hidden", false)
        $('form#create').append($("input#form").val())
        $('button#create').attr("hidden", true)
        $('button#cancel').attr("hidden", false)
        $('button#selectAll').attr("hidden", true)
        $('form#create').attr("action", model)
        $('form#get').attr("hidden", false)
        $('form#get').attr("action", model + '/' + data[model][0].id)
        // alert(model)
    })
    ...
    $('button#cancel').click(function(){
        ...
        $('button#selectAll').attr("hidden", false)
    ...
    // $('#tree').treeview('onNodeChecked', $('button#cancel').attr("hidden", false))
    ...
    function generateChildNode(object, parentNode, relationMappingsKey){
        $.each(object[relationMappingsKey], function (index, relationMappingObject) {
            var childNode = generateNode(relationMappingObject);
            parentNode.nodes.push(childNode);
            // generateNodes(relationMappingObject, [])
            // parentNode.nodes.push(childNode);
        });
    }
    ...
    function generateNodes(object, arr_tree) {
        ...
        if(relationMappingsKey === ""){
            parentNode = generateNode(object)
        }
        else{
            parentNode.nodes = []
            parentNode = generateNode(object)
            generateChildNode(object, parentNode, relationMappingsKey)
        }

        arr_tree.push(parentNode);

        var $searchableTree = $('#tree').treeview({ data: arr_tree,
                                multiSelect: false,
                                showCheckbox: true,
                                onNodeChecked: function (event, data) { $('button#cancel').attr("hidden", false); $('button#delete').attr("hidden", false); $('button#selectAll').attr("hidden", false) },
                                onNodeUnchecked: function (event, data) { $('button#cancel').attr("hidden", true); $('button#delete').attr("hidden", true) },
